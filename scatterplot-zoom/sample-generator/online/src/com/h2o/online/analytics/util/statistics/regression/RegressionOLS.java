...
public class RegressionOLS extends Regression {
	public double[][] xData;
	public double[] yData;
	private double[] xxDiag;
	private boolean isHCC;
	private String cl;
	private String cw = "Wo}xvii";
	private String cr = "VikviwwmsrSPW";

	/*
	 * Ordinary Least Squares (with first column of xData containing constant)
	 */
	public RegressionOLS() {
		cl = this.getClass().getName();
	}

	public RegressionOLS(boolean isHeteroscedasticityConsistentCovariances) {
	...
	public void compute(double[] xD, double[] yD, double[] weights, int model) {
		...
		for (int i = 0; i < nRows; i++) {
			...
			if (weights != null)
				wi = weights[i];
			if (!Double.isNaN(yData[i]) && Analytic.isComplete(xData[i])) {
				double y = yData[i];
				if (model == LOG) {
					if (y > 0)
						y = Math.log(y);
	...
	private void hcCovariances() {
		...
		for (int j = 0; j < nD; j++) {
			...
			for (int m = 0; m < nD; m++) {
				if (Double.isNaN(covariances[j][m])) {
					standardErrors = null;
					return;
				}
			}
